    ...
    <div class="container quiz-container">
        ...
        <div class="quiz-header">
            <div class="quiz-header-content">
                <h1 class="quiz-title-main">{{ $quizData['title'] }}</h1>
                ...
                <div class="quiz-meta">
                    <div class="quiz-meta-item">
                        <span>📝</span>
                        <span>{{ $quizData['total_questions'] }} Questions</span>
                    </div>
                    <div class="quiz-meta-item">
                        <span>⏱️</span>
                    ...
                    <div class="quiz-meta-item">
                        <span>🎯</span>
                        <span>Pass: {{ $quizData['passing_score'] }}%</span>
                    </div>
                    <div class="quiz-meta-item">
                        <span>🔢</span>
                        <span>{{ ucfirst($quizData['level']) }}</span>
                    </div>
                </div>
            </div>
        </div>
        ...
        <div class="timer-section">
            <div class="progress-info">
                <span id="answeredCount">0</span> / {{ $quizData['total_questions'] }} answered
            </div>
            <div class="timer" id="timer">
                ⏱️ <span id="timeLeft">{{ $quizData['time_limit'] * 60 }}</span>
            </div>
        </div>

        <!-- Quiz Form -->
        <form action="{{ route('client.quizzes.submit.dynamic', $sessionKey) }}" method="POST" id="quizForm">
            @csrf
            ...
            @foreach($quizData['questions'] as $index => $question)
                <div class="question-card">
                    <div class="question-header">
                        ...
                        <span class="question-points">{{ $question['points'] }} {{ $question['points'] > 1 ? 'pts' : 'pt' }}</span>
                    </div>
...
@push('scripts')
<script>
    let timeLeft = {{ $quizData['time_limit'] * 60 }};
